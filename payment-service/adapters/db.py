...
class DatabaseAdapter(PaymentStorageRepository):
    ...
    async def save_payment(self, payment: PaymentResponse) -> None:
        ...
        try:
            existing = (
                self.db.query(PaymentRecord).filter_by(payment_id=payment.id).first()
            )
            ...
            if existing:
                ...
                if payment.metadata:
                    ...
                    existing.virtual_account_number = payment.metadata.get(
                ...
                if payment.metadata:
                    ...
                    new_payment.virtual_account_number = payment.metadata.get(
                        "virtual_account_number"
                    )
    ...
    async def get_payment(self, payment_id: str) -> Optional[PaymentResponse]:
        ...
        try:
            record = (
                self.db.query(PaymentRecord).filter_by(payment_id=payment_id).first()
            )
